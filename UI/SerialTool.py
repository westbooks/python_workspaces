...
class SerialToolUI(object):
    ...
    def create_frm_left(self):
        ...
        self.frm_left_label = pytk.PyLabel(self.frm_left, text='串口罗列', font=font)
        self.frm_left_listbox = pytk.PyListBox(self.frm_left, height=size_dict["list_box_height"], font=font)
        self.frm_left_serial_set = pytk.PyLabelFrame(self.frm_left)
        self.frm_left_btn = pytk.PyButton(self.frm_left, text='Open', font=font, command=self.Toggle)
    ...
    def create_frm_left_serial_set(self):
        ...
        setting_label_list = ["波特率 ：", "校验位 ：", "数据位 ：", "停止位 ："]
        baudrate_list = ["1200", "2400", "4800", "9600", "14400", "19200", "38400", "43000", "57600", "76800", "115200",
                         "12800"]
        ...
        for index, item in enumerate(setting_label_list):
            frm_left_label_temp = pytk.PyLabel(self.frm_left_serial_set, text=item, font=('Monaco', 10))
            frm_left_label_temp.grid(row=index, column=0, padx=1, pady=2, sticky='e')

        self.frm_left_combobox_baudrate = ttk.Combobox(self.frm_left_serial_set,width=15,values=baudrate_list)
    ...
    def create_frm_right_reset(self):
        ...
        self.frm_right_reset_label = pytk.PyLabel(self.frm_right_reset,
                                                  text="数据发送区" + " " * size_dict["reset_label_width"],
                                                  font=font)
    ...
    def create_frm_right_clear(self):
        ...
        self.frm_right_clear_label = pytk.PyLabel(self.frm_right_clear,
                                                  text="数据接收区" + " " * size_dict["clear_label_width"],
                                                  font=font)
